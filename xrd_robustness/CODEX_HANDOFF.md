...
> **2026-07-26 run-5 recovery override:** The registered queue completed 4/7 runs. Residual λ=0.2 then completed all 50 epochs and 30,650 optimizer steps but exited before `results.json` because its Train-only posthoc residual-probe manifest used the non-contract split label `posthoc_train`. The traceback, manifests, history, stream audit, and epoch-50 checkpoint remain intact; no later run started. The engineering repair maps that diagnostic manifest to the valid scientific split `train`, with a regression test, while preserving the frozen model/grid/seed/budget/exposure and training hashes. Resume is authorized only from the verified epoch-50 checkpoint after tests plus commit/push; finish this same run before runs 6–7. Formal 15-run, simulated Test, real XRD, real adaptation, and V10 remain locked.
