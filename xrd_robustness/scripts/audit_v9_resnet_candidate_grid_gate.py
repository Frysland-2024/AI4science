#!/usr/bin/env python3
"""Train-only Gate for the ResNet V9 lambda-grid revision.

The audit rebuilds the learned Dynamic/Paired ERM ResNet-18-GN state from epoch 0
...
from audit_v9_resnet_learned_state_scale import (  # noqa: E402
    BATCH_SIZE,
    CRYSTAL_SYSTEMS,
    LEARNING_RATE,
    PROBE_EPOCHS,
...
def _assert_registered_proposal(governance: Mapping[str, Any]) -> None:
    if governance.get("audit_probe_grids") != APPROVED_GRIDS:
        raise ValueError("governance does not contain the registered audit probe grids")
    ...
    if revision.get("completed_range_revisions") != 0:
        raise ValueError("ResNet candidate range must remain unrevised before this Gate")
    if governance.get("candidate_range_frozen_for_validation") is not False:
        raise ValueError("ResNet candidate range must be unfrozen before this Gate")
...
def run_audit(
    *,
    ...
    return {
        "schema_version": "v9-resnet-candidate-grid-gate-v1",
        "status": "pass" if all(checks.values()) else "fail",
        ...
        "audited_probe_grids": APPROVED_GRIDS,
        "probe_grid_inherited_as_resnet_candidates": False,
        ...
        "input_hashes": {
            ...
            "learned_state_audit_implementation": file_hash(
                PROJECT_ROOT / "scripts" / "audit_v9_resnet_learned_state_scale.py"
            ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default=str(PROJECT_ROOT / "reports" / "v9_resnet_candidate_grid_gate.json"),
    )
