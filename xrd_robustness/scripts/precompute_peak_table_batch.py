#!/usr/bin/env python3
"""Compute ideal peak tables in the compatible pymatgen environment."""
...
def main() -> int:
    ...
    for item in payload:
        try:
            ...
            rows.append({"material_id": item["material_id"], "error": str(error)})
    Path(args.output).write_text(json.dumps(rows, separators=(",", ":")), encoding="utf-8")
    return 0
