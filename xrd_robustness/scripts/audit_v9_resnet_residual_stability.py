#!/usr/bin/env python3
"""Pre-registered Train-only ResNet residual-probe stability audit.

This diagnostic trains one method-neutral Dynamic/Paired ERM ResNet-18-GN model
...
MILESTONES = (3, 5, 10)
TRAIN_EPOCHS = 10
STABILITY_SEEDS = (20260722, 20260723, 20260724)
BATCH_SIZE = 16
PROBE_PER_CLASS = 100
SCALE_PER_CLASS = 100
PROBE_EPOCHS = 50
...
def _fit_and_evaluate_probe(
    ...
    *,
    ...
    confusion = [
        [
            int(((targets == actual) & (predictions == predicted)).sum())
            for predicted in range(len(CRYSTAL_SYSTEMS))
        ]
        for actual in range(len(CRYSTAL_SYSTEMS))
    ]
    per_class_f1 = {
        system: float(
            0.0
            if (denominator := 2 * confusion[index][index]
                + sum(confusion[other][index] for other in range(len(CRYSTAL_SYSTEMS)) if other != index)
                + sum(confusion[index][other] for other in range(len(CRYSTAL_SYSTEMS)) if other != index)) == 0
            else 2 * confusion[index][index] / denominator
        )
        for index, system in enumerate(CRYSTAL_SYSTEMS)
    }
    status = (
        "signal_demonstrated"
    ...
    return probe, {
        ...
        "audit_per_class_f1": per_class_f1,
        "audit_confusion_matrix": confusion,
...
def run_audit(
    *,
    ...
    model = ML4PXRDResNet1D(ML4PXRDResNet1DConfig(model_id="18")).to(device)
    ...
    checks = {
        ...
        "formal_resnet18_gn_used": model.config.model_id == "18",
    ...
    return {
        "schema_version": "v9-resnet-residual-stability-seed-audit-v1",
        ...
        "training_protocol": {
            ...
            "backbone": "ML4PXRDResNet1D-18-GN",
            "epochs": TRAIN_EPOCHS,
        ...
        "input_hashes": {
            ...
            "backbone_source": file_hash(
                PROJECT_ROOT / "src" / "xrd_robustness" / "models" / "ml4pxrd_resnet1d.py"
            ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--seeds",
        default=",".join(str(seed) for seed in STABILITY_SEEDS),
        help="comma-separated fixed Train-only diagnostic seeds",
    )
    parser.add_argument(
        "--output",
        default=str(
            PROJECT_ROOT / "reports" / "v9_resnet_residual_stability_audit.json"
        ),
    )
    args = parser.parse_args()
    seeds = tuple(int(value) for value in args.seeds.split(",") if value.strip())
    if seeds != STABILITY_SEEDS:
        raise SystemExit(f"stability audit requires exactly the preregistered seeds {STABILITY_SEEDS}")
    reports = [
        run_audit(seed=seed, device_name=args.device, worker_count=args.prefetch_workers, prefetch_batches=args.prefetch_batches)
        for seed in seeds
    ]
    pass_counts = {
        str(milestone): sum(
            report["milestones"][str(milestone)]["residual_probe_gate"]["status"] == "signal_demonstrated"
            for report in reports
        )
        for milestone in MILESTONES
    }
    required_milestones = (5, 10)
    stability_pass = all(pass_counts[str(milestone)] >= 2 for milestone in required_milestones)
    report = {
        "schema_version": "v9-resnet-residual-stability-audit-v1",
        "scope": "train_only_residual_probe_stability",
        "preregistered_seeds": list(seeds),
        "milestones": list(MILESTONES),
        "decision_rule": "at least two of three seeds must demonstrate the fixed residual probe at both epochs 5 and 10",
        "residual_probe_pass_counts": pass_counts,
        "status": "stable_signal_demonstrated" if stability_pass else "stable_signal_not_demonstrated",
        "candidate_range_reopening_authorized": False,
        "seven_run_started": False,
        "validation_used": False,
        "simulated_test_used": False,
        "real_xrd_used": False,
        "input_hashes": {
            "audit_script": file_hash(Path(__file__).resolve()),
            "preregistration": file_hash(
                PROJECT_ROOT / "configs" / "v9_resnet_residual_stability.preregistered.json"
            ),
        },
        ...
        json.dumps(
            {
                ...
                "residual_probe_pass_counts": report["residual_probe_pass_counts"],
                "output": str(output),
                "seven_run_started": report["seven_run_started"],
            },
            indent=2,
        ),
        flush=True,
