...
async function main() {
  ...
  const inspected = await presentation.inspect({ kind: "slide,textbox,shape,table,chart,notes,layout", maxChars: 80000 });
  ...
  if (target?.text !== expectedText) throw new Error(`P4 text mismatch after roundtrip: ${target?.text}`);
  if (JSON.stringify(target.bbox) !== JSON.stringify([72, 446, 420, 190])) throw new Error("P4 textbox geometry changed");
  const table = p4.find((entry) => entry.id === "tb/mhovq5k3");
  if (!table || JSON.stringify(table.bbox) !== JSON.stringify([545, 142, 655, 470])) throw new Error("P4 table changed");
  ...
  await fs.writeFile(path.join(tempDir, "roundtrip-p4-inspect.ndjson"), `${p4.map((entry) => JSON.stringify(entry)).join("\n")}\n`, "utf8");
