...
它仍属于 [`A-METRO`](GRADUATE_RESEARCH_DIRECTION.md#route-framework) 的“测量结果 -> 物理信息推断”，同时开始触及以物理模型、参数识别和模拟器校准为中心的 `B-DEVICE` 式方法结构。
...
详细文献、代码与数据清单已合并到本文的[资源附录](#resource-manifest)。
...
<a id="resource-manifest"></a>

## 附录 A：论文、代码、数据与补充材料资源清单

以下内容由原资源清单并入正式执行计划，避免计划与资源清单分成两个文件。
...
This document is the formal execution plan. It deliberately narrows the first implementation to a known-phase, single-phase tetragonal task. The broader resources in this appendix are references for staged expansion, not a commitment to reproduce all listed systems in V0.
