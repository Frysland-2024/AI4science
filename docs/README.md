# 项目文档

这个目录放项目当前状态、对外讲法和完整历史档案。

| 文件 | 用途 |
|---|---|
| [`CURRENT_STATE.md`](CURRENT_STATE.md) | 当前科学设计、做完的事、实验进度、卡点和下一步 |
| [`APPLICATION_RESEARCH_NARRATIVE.md`](APPLICATION_RESEARCH_NARRATIVE.md) | 申请材料、面试和项目介绍的统一说法 |
| [`PROJECT_HISTORY.md`](PROJECT_HISTORY.md) | 完整研究演化档案：保留失败、版本变化、思路转向与历史决策；用于追溯项目如何发展，不代表当前方法或论文主张 |
| [`../xrd_robustness/MANUSCRIPT.md`](../xrd_robustness/MANUSCRIPT.md) | 论文正文框架 |
| [`../xrd_robustness/reports/RESULTS.md`](../xrd_robustness/reports/RESULTS.md) | 模拟结果汇总 |

第一次阅读建议从 `CURRENT_STATE.md` 开始；只有需要追溯某个决定、失败实验或思路变化时，再查 `PROJECT_HISTORY.md`。

对外的当前说法以 `CURRENT_STATE.md` 和结果文件为准。项目做的是七晶系 PXRD 分类、在线物理扰动，以及 Dynamic ERM 和 Dynamic JS 的配对比较。
