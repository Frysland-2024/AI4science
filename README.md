...
- simulated Test、real XRD、real-domain adaptation 和 V10 仍锁定。
...
2. `xrd_robustness/reports/v9_resnet_js_ten_run_summary.json` 是 ten-run
   replication 的权威结果记录。
3. `xrd_robustness/CODEX_HANDOFF.md` 规定工程接管、授权和 Test/real-XRD
   访问边界。
4. `00_project_context/CURRENT_STATE.md` 汇总当前科学身份、实验进度、阻塞
   与下一步。
5. `00_project_context/PROJECT_JOURNEY.md` 记录方案为什么改变；历史内容不得
   因当前方案改变而删除。
6. 日期化报告与 Git 历史保留中间状态，但不得覆盖当前权威记录。
7. `04_external_lab_data/` 是 Git 忽略的本地外部实验资料区；原始光谱、PDF、
   图片、实验表单和外部脚本不得提交。
8. datasets、outputs、checkpoints、generated spectra、caches、环境目录和凭据
   不进入 Git。

## 下一阶段

当前不应重复训练 ten-run，也不应继续调参。下一步是冻结 one-shot
simulated-Test evaluation contract，包括 checkpoint rule、指标、随机性、输出
目录、审计和 no-retuning policy；只有获得新的明确授权后才能访问 simulated
Test。real XRD 必须作为更后的独立 external-validation stage。
